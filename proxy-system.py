...
print(Fore.CYAN) 
slowprint("Lutfen Lisans Kodunuzu Giriniz: ", 0.5)
lisans = input("-> ")
lisansapi = "http://api.sosyalmedyam.web.tr/apikey/" + lisans
response = requests.get(lisansapi)
json_str = json.dumps(response.json()) 
resp = json.loads(json_str)

if (resp['message'] == False):
    print(Fore.RED)
    slowprint("Lisans Kodunuz Yanlış!", 0.2)
    access = False
    time.sleep(10)
    os._exit(0)
else:
    print(Fore.CYAN)
    slowprint("Lisans Kodunuz Doğru!", 0.2)
    time.sleep(2)
    os.system("cls")
